...
LED_COUNT = 167				# Set to the number of LEDs in your installation
LED_PIN = board.D12  		# Pin used for data for the LEDs
LED_BRIGHTNESS = .5 		# Default/daytime brightness. Value between 0 and 1
LIGHTNING_BRIGHTNESS = 0.5	# Brightness when flashing white for lightning animation. Change in def lightning_animation(leds):
...
COLOR_SNOW = [125, 125, 0]			#Yellow				#Off
COLOR_LIGHTNING = [255, 255, 255]		#White
COLOR_WINDY = [25, 0, 0]			#Dim Green for windy
COLOR_MISSING = [110, 255, 0]			#Missing Airport Data Orange

#--------Animation Parameters--------
dim_brightness = 0.02			# Brightness the LEDs will dim to when doing the windy animation
threshold_wind_speed = 14		# Airports with wind speed above this value will animate for wind
windy_animation_dim_pause = 1.7	# Time LED's should stay dim before going bright again
wind_fade_time = 0.5			# Speed of LED fade for wind animation
num_steps = 100					# Amount of steps for dimming animation
animation_pause = 30			# Time between animation sets
...
SNOW_ANIMATION = False				# Turn snow animation on or off
...
LED_BRIGHTNESS_DIM = .15					# Led brightness after sunset or DIM_TIME_START
USE_SUNRISE_SUNSET = False					# Use sunrise or sunset times for LED dimming
BRIGHT_TIME_START = datetime.time(7, 00)	# Custom time to set LEDs to be bright
DIM_TIME_START = datetime.time(19, 00)		# Custom time to set LEDs to dim
