...
def validate_float_field(field_name, min_value, max_value, default_value):
    try:
        ...
        if value < min_value:
            error_message = f"{field_name.replace('_', ' ').title()} was too low. Please enter a value between {min_value} and {max_value}."
...
def index():
    ...
    if request.method == 'POST':
        ...
        if 'update' in request.form:
            ...
            subprocess.run(["sudo", "/home/pi/metar/bin/python3", "kill.py"])
